...
params: 
    type: object
"""
async def task(self, url=None, method="get", port=None, data=None, headers={}, params=None, timeout=5):
    if isinstance(url, str):
        url = urlparse(url if url.startswith('http') else 'https://{}'.format(url))

    async with aiohttp.ClientSession() as session:
        async with session.request(method, url.geturl(), data=data, headers=headers, timeout=timeout) as response:
            try:
